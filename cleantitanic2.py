...
def phase2clean(train, test):
    ...
    data_type_dict={'Pclass':'ordinal', 'Sex':'nominal', 
                    ...
                    'Deck':'nominal', 'Family_Size':'ordinal', 'Fare_Per_Person':'numeric'}
...
def discretise_numeric(train, test, data_type_dict):
    N=len(train)
    M=len(test)
    ##Renames index to make test/train continuous
...
def clean():
    ...
    traindf = pd.read_csv('../Documents/Kaggle/Titanic/train.csv', header=0)
    testdf = pd.read_csv('../Documents/Kaggle/Titanic/test.csv', header=0)
    ...
    predictiondf.to_csv('../Documents/Kaggle/Titanic/prediction.csv',
                  index=False)
